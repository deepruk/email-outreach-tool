...
const defaultSteps = (columns: string[]): SequenceStepInput[] => [
  ...
  { key: "follow_up_1", label: "Follow-up 1", subject_column: guess(columns, ["followup1subject", "followupsubject1"]), body_column: guess(columns, ["followup1body", "followupbody1"]), day_offset: 3, send_time: "11:00" },
  { key: "follow_up_2", label: "Follow-up 2", subject_column: guess(columns, ["followup2subject", "followupsubject2"]), body_column: guess(columns, ["followup2body", "followupbody2"]), day_offset: 6, send_time: "14:00" },
];
...
export default function CsvCampaignWizard() {
  ...
  const payload = useMemo<CsvCampaignCreate | null>(() => source ? {
    ...
    steps: sequence,
    timezone,
  } : null, [source, name, emailColumn, firstNameColumn, companyColumn, statusColumn, selectedInboxes, sequence, timezone]);
  ...
  const goNext = () => {
    ...
    if (step === 3) return previewMutation.mutate(payload);
