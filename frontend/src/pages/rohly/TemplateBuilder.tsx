...
import { ArrowLeft, ArrowRight, Braces, Check, Clock3, Plus, Rocket, Trash2 } from "lucide-react";
...
const variables = ["{{first_name}}", "{{company}}", "{{email}}", "{{name}}", "{{job_title}}", "{{industry}}", "{{city}}", "{{country}}"];
    ...
    <div className="grid gap-4 xl:grid-cols-[1.35fr_.65fr]">
      <div className="space-y-4">
        <Surface className="p-5" testId="template-campaign-details">
          ...
          <label className="mt-4 block text-xs font-semibold">Campaign name<Input value={name} onChange={(event) => setName(event.target.value)} className="mt-2 h-10" /></label>
        </Surface>

        <Surface className="p-5" testId="template-campaign-recipients">
          <div className="flex items-center justify-between"><div><h2 className="font-heading text-lg font-medium">Recipients</h2><p className="mt-1 text-xs text-slate-500">Choose the contacts who will receive the sequence.</p></div><span className="text-xs font-semibold text-blue-700">{selectedRecipients.length} selected</span></div>
          <div className="mt-4 max-h-64 divide-y divide-slate-100 overflow-auto rounded-md border border-slate-200">
            {recipients.length ? recipients.map((recipient) => <label key={recipient.id} className="flex cursor-pointer items-center gap-3 p-3 hover:bg-slate-50"><input type="checkbox" checked={selectedRecipients.includes(recipient.id)} onChange={() => toggle(setSelectedRecipients, recipient.id)} className="size-4 accent-blue-700" /><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium">{recipient.name}</p><p className="truncate text-xs text-slate-500">{recipient.email}{recipient.company ? ` · ${recipient.company}` : ""}</p></div></label>) : <p className="p-6 text-center text-sm text-slate-500">No recipients yet. Add contacts from Imports first.</p>}
          </div>
        </Surface>

        <Surface className="p-5" testId="template-campaign-inboxes">
          <div className="flex items-center justify-between"><div><h2 className="font-heading text-lg font-medium">Sending inboxes</h2><p className="mt-1 text-xs text-slate-500">Emails are distributed round-robin across the selected connected inboxes.</p></div><span className="text-xs font-semibold text-blue-700">{selectedInboxes.length} selected</span></div>
        ...
        <Surface className="p-5" testId="template-campaign-sequence">
          <div className="flex items-center justify-between"><div><h2 className="font-heading text-lg font-medium">Email sequence</h2><p className="mt-1 text-xs text-slate-500">Choose a reusable Rohly template for each step.</p></div><Button variant="outline" size="sm" onClick={addFollowUp} className="gap-1"><Plus size={14} /> Add follow-up</Button></div>
          <div className="mt-5 space-y-3">
            {steps.map((step, index) => { const template = selectedTemplates[step.template_id]; return <div key={`${index}-${step.template_id}`} className="rounded-lg border border-slate-200 p-4">
              <div className="flex items-center gap-3"><div className="flex size-7 items-center justify-center rounded-full bg-slate-100 text-xs font-bold">{index + 1}</div><div className="flex-1"><p className="text-sm font-semibold">{step.label}</p>{index > 0 && <p className="text-[11px] text-slate-500">Days after previous email</p>}</div>{index > 0 && <Button variant="ghost" size="icon" onClick={() => removeStep(index)}><Trash2 size={15} /></Button>}</div>
              <div className="mt-4 grid gap-3 sm:grid-cols-[1fr_150px]"><label className="block text-xs font-semibold">Template<select value={step.template_id} onChange={(event) => updateStep(index, { template_id: event.target.value })} className="mt-2 h-10 w-full rounded-md border border-slate-200 bg-white px-3 text-sm"><option value="">Select a saved template</option>{templates.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>{index > 0 && <label className="block text-xs font-semibold">Delay<Input type="number" min={1} value={step.delay_days} onChange={(event) => updateStep(index, { delay_days: Number(event.target.value) })} className="mt-2 h-10" /></label>}</div>
              {template && <div className="mt-3 rounded-md bg-slate-50 p-3"><p className="text-xs font-semibold">{previewValue(template.subject, selectedPreview)}</p><p className="mt-2 whitespace-pre-wrap text-xs leading-5 text-slate-600">{previewValue(template.body, selectedPreview)}</p></div>}
            </div>; })}
          </div>
        </Surface>
      ...
      <div className="space-y-4">
        <Surface className="sticky top-4 p-5" testId="template-campaign-preview">
          <div className="flex items-center gap-2"><Braces size={15} /><h2 className="font-heading text-lg font-medium">Personalization preview</h2></div>
          <select value={previewRecipient} onChange={(event) => setPreviewRecipient(event.target.value)} className="mt-4 h-10 w-full rounded-md border border-slate-200 bg-white px-3 text-sm"><option value="">Preview sample lead</option>{recipients.map((recipient) => <option key={recipient.id} value={recipient.id}>{recipient.name} · {recipient.company || recipient.email}</option>)}</select>
          {steps[0]?.template_id && selectedTemplates[steps[0].template_id] ? <div className="mt-5 rounded-lg border border-slate-200 p-4"><p className="text-[10px] font-bold uppercase tracking-[0.1em] text-slate-400">Initial email</p><p className="mt-3 text-sm font-semibold">{previewValue(selectedTemplates[steps[0].template_id].subject, selectedPreview)}</p><div className="mt-4 whitespace-pre-wrap text-sm leading-6 text-slate-700">{previewValue(selectedTemplates[steps[0].template_id].body, selectedPreview)}</div></div> : <div className="mt-5 rounded-lg border border-dashed border-slate-300 p-6 text-center text-sm text-slate-500">Select a template to preview the personalized email.</div>}
          <div className="mt-5 rounded-md bg-blue-50 p-3 text-xs leading-5 text-blue-800"><strong>Automatic:</strong> every recipient gets the same template, with their own name, company, email and other supported variables filled in before sending.</div>
          <Button onClick={validateAndLaunch} disabled={createMutation.isPending} className="mt-5 w-full gap-2 bg-blue-700 hover:bg-blue-800"><Rocket size={15} /> {createMutation.isPending ? "Scheduling campaign…" : "Launch campaign"} <ArrowRight size={14} /></Button>
          <Link to="/campaigns/new" className="mt-3 flex items-center justify-center gap-1 text-xs font-semibold text-slate-500"><ArrowLeft size={12} /> Back to campaign source</Link>
        </Surface>
      </div>
    </div>
  </div>;
