...
import { ArrowLeft, ArrowRight, Braces, Check, Clock3, FileSpreadsheet, Plus, Rocket, Trash2, X } from "lucide-react";
...
export default function TemplateBuilder() {
  ...
  const inboxesQuery = useQuery({ queryKey: ["rohly-campaign-inboxes"], queryFn: () => apiGet<Inbox[]>("/workspace/rohly-campaign-inboxes") });
  ...
  return <div data-testid="template-campaign-wizard-page">
    ...
    {listPickerOpen && <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 p-4" role="dialog" aria-modal="true" aria-label="Choose recipient list"><div className="w-full max-w-2xl rounded-xl border border-slate-200 bg-white shadow-2xl"><div className="flex items-center justify-between border-b border-slate-200 px-5 py-4"><div><h2 className="text-lg font-semibold">Choose a recipient list</h2><p className="mt-1 text-xs text-slate-500">Select an existing CSV import. Its contacts will be added to this campaign.</p></div><Button type="button" variant="ghost" size="icon" onClick={() => setListPickerOpen(false)}><X size={17} /></Button></div><div className="max-h-[60vh] overflow-auto p-5">{listsQuery.isLoading ? <p className="py-10 text-center text-sm text-slate-500">Loading imported lists…</p> : lists.length ? <div className="space-y-2">{lists.map((list) => { const active = selectedLists.includes(list.id); const busy = useListMutation.isPending && useListMutation.variables === list.id; return <div key={list.id} className={`flex items-center gap-3 rounded-lg border p-4 ${active ? "border-blue-300 bg-blue-50/50" : "border-slate-200"}`}><div className="flex size-9 items-center justify-center rounded-md bg-emerald-50 text-emerald-700"><FileSpreadsheet size={15} /></div><div className="min-w-0 flex-1"><p className="truncate text-sm font-semibold">{list.filename}</p><p className="mt-1 text-xs text-slate-500">{list.row_count} contacts · {list.columns.slice(0, 5).join(" · ")}{list.columns.length > 5 ? " …" : ""}</p></div><Button type="button" size="sm" variant={active ? "outline" : "default"} disabled={busy} onClick={() => useListMutation.mutate(list.id)} className="gap-1">{active ? <><Check size={13} /> Added</> : busy ? "Adding…" : "Add list"}</Button></div>; })}</div> : <div className="py-10 text-center"><p className="text-sm font-medium text-slate-700">No imported lists yet</p><p className="mt-1 text-xs text-slate-500">Import a CSV first, then return here and choose the list without leaving the campaign.</p><Link to="/imports" onClick={() => setListPickerOpen(false)} className="mt-4 inline-flex items-center gap-1 text-xs font-semibold text-blue-700"><Plus size={13} /> Go to Imports</Link></div>}</div><div className="flex justify-end border-t border-slate-200 px-5 py-3"><Button type="button" variant="outline" onClick={() => setListPickerOpen(false)}>Done</Button></div></div></div>}
  </div>;
}
