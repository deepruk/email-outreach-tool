  ...
  return <div data-testid="template-campaign-wizard-page">
    ...
    <div className="grid gap-4 xl:grid-cols-[1.35fr_.65fr]">
      <div className="space-y-4">
        ...
        <Surface className="p-5" testId="template-campaign-recipients"><div className="flex items-center justify-between gap-4"><div><h2 className="font-heading text-lg font-medium">Recipients</h2><p className="mt-1 text-xs text-slate-500">Choose the contacts who will receive the sequence.</p></div><div className="flex items-center gap-3"><Link to="/imports" className="inline-flex items-center gap-1 rounded-md border border-slate-200 px-3 py-1.5 text-xs font-semibold text-slate-700 hover:bg-slate-50"><Plus size={13} /> Add recipient list</Link><span className="text-xs font-semibold text-blue-700">{selectedRecipients.length} selected</span></div></div><div className="mt-4 max-h-64 divide-y divide-slate-100 overflow-auto rounded-md border border-slate-200">{recipients.length ? recipients.map((recipient) => <label key={recipient.id} className="flex cursor-pointer items-center gap-3 p-3 hover:bg-slate-50"><input type="checkbox" checked={selectedRecipients.includes(recipient.id)} onChange={() => toggle(setSelectedRecipients, recipient.id)} className="size-4 accent-blue-700" /><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium">{recipient.name}</p><p className="truncate text-xs text-slate-500">{recipient.email}{recipient.company ? ` · ${recipient.company}` : ""}</p></div></label>) : <div className="p-6 text-center"><p className="text-sm text-slate-500">No recipients yet.</p><Link to="/imports" className="mt-2 inline-flex items-center gap-1 text-xs font-semibold text-blue-700 hover:text-blue-800"><Plus size={13} /> Add contacts from Imports</Link></div>}</div></Surface>
