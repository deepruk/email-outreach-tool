...
export default function TemplateBuilder() {
  ...
  const createTemplateMutation = useMutation({
    mutationFn: () => apiPost<Template>("/workspace/templates", { name: newTemplateName.trim(), subject: newTemplateSubject, body: newTemplateBody }),
  ...
  const refreshLists = () => listsQuery.refetch();
  const refreshInboxes = () => inboxesQuery.refetch();
  ...
  const openTemplateCreator = (stepIndex: number) => { setTemplateCreatorStep(stepIndex); setNewTemplateName(""); setNewTemplateSubject(""); setNewTemplateBody(""); setTemplateCreatorOpen(true); };
  ...
  return <div data-testid="template-campaign-wizard-page">
    ...
    <div className="grid gap-4 xl:grid-cols-[1.35fr_.65fr]">
      <div className="space-y-4">
        ...
        <Surface className="p-5" testId="template-campaign-inboxes"><div className="flex items-center justify-between gap-4"><div><h2 className="font-heading text-lg font-medium">Sending inboxes</h2><p className="mt-1 text-xs text-slate-500">Select the Gmail inboxes that can send this campaign. Emails are distributed round-robin.</p></div><div className="flex items-center gap-2"><Button type="button" variant="outline" size="sm" onClick={refreshInboxes} className="gap-1"><RefreshCw size={13} /> Refresh</Button><Link to="/inboxes" className="inline-flex items-center gap-1 rounded-md border border-slate-200 px-3 py-1.5 text-xs font-semibold text-slate-700 hover:bg-slate-50"><Plus size={13} /> Connect inbox</Link><span className="text-xs font-semibold text-blue-700">{selectedInboxes.length} selected</span></div></div><div className="mt-4 space-y-2">{inboxes.length ? inboxes.map((inbox) => <label key={inbox.id} className={`flex cursor-pointer items-center gap-3 rounded-md border p-3 hover:bg-slate-50 ${selectedInboxes.includes(inbox.id) ? "border-blue-300 bg-blue-50/40" : "border-slate-200"}`}><input type="checkbox" checked={selectedInboxes.includes(inbox.id)} onChange={() => toggle(setSelectedInboxes, inbox.id)} className="size-4 accent-blue-700" /><div className="flex-1"><p className="text-sm font-medium">{inbox.display_name}</p><p className="text-xs text-slate-500">{inbox.email}</p></div><span className="text-xs text-slate-500">{inbox.daily_sending_limit}/day</span></label>) : <div className="p-5 text-center"><p className="text-sm text-slate-500">No connected Gmail inboxes found.</p><Link to="/inboxes" className="mt-2 inline-flex items-center gap-1 text-xs font-semibold text-blue-700"><Plus size={13} /> Connect a Gmail inbox</Link></div>}</div></Surface>
        <Surface className="p-5" testId="template-campaign-recipients"><div className="flex items-center justify-between gap-4"><div><h2 className="font-heading text-lg font-medium">Recipients</h2><p className="mt-1 text-xs text-slate-500">Choose individual contacts or add an existing imported list.</p></div><div className="flex items-center gap-3"><Button type="button" variant="outline" size="sm" onClick={() => setListPickerOpen(true)} className="gap-1"><FileSpreadsheet size={13} /> Add recipient list</Button><span className="text-xs font-semibold text-blue-700">{selectedRecipients.length} selected</span></div></div>{selectedLists.length > 0 && <div className="mt-3 flex flex-wrap gap-2">{selectedLists.map((sourceId) => { const list = lists.find((item) => item.id === sourceId); return <span key={sourceId} className="inline-flex items-center gap-1 rounded-full bg-blue-50 px-2.5 py-1 text-[11px] font-semibold text-blue-700"><Check size={11} /> {list?.filename || "Imported list"}</span>; })}</div>}<div className="mt-4 max-h-64 divide-y divide-slate-100 overflow-auto rounded-md border border-slate-200">{recipients.length ? recipients.map((recipient) => <label key={recipient.id} className="flex cursor-pointer items-center gap-3 p-3 hover:bg-slate-50"><input type="checkbox" checked={selectedRecipients.includes(recipient.id)} onChange={() => toggle(setSelectedRecipients, recipient.id)} className="size-4 accent-blue-700" /><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium">{recipient.name}</p><p className="truncate text-xs text-slate-500">{recipient.email}{recipient.company ? ` · ${recipient.company}` : ""}</p></div></label>) : <div className="p-6 text-center"><p className="text-sm text-slate-500">No recipients yet.</p><Button type="button" variant="outline" size="sm" onClick={() => setListPickerOpen(true)} className="mt-3 gap-1"><Plus size={13} /> Choose an imported list</Button></div>}</div></Surface>
        <Surface className="p-5" testId="template-campaign-sequence"><div className="flex items-center justify-between gap-4"><div><h2 className="font-heading text-lg font-medium">Email sequence</h2><p className="mt-1 text-xs text-slate-500">Choose a reusable Rohly template for each step, or create one now.</p></div><div className="flex items-center gap-2"><Button variant="outline" size="sm" onClick={() => openTemplateCreator(steps.length - 1)} className="gap-1"><Plus size={14} /> Create template</Button><Button variant="outline" size="sm" onClick={addFollowUp} className="gap-1"><Plus size={14} /> Add follow-up</Button></div></div><div className="mt-5 space-y-3">{steps.map((step, index) => { const template = selectedTemplates[step.template_id]; return <div key={`${index}-${step.template_id}`} className="rounded-lg border border-slate-200 p-4"><div className="flex items-center gap-3"><div className="flex size-7 items-center justify-center rounded-full bg-slate-100 text-xs font-bold">{index + 1}</div><div className="flex-1"><p className="text-sm font-semibold">{step.label}</p>{index > 0 && <p className="text-[11px] text-slate-500">Days after previous email</p>}</div>{index > 0 && <Button variant="ghost" size="icon" onClick={() => removeStep(index)}><Trash2 size={15} /></Button>}</div><div className="mt-4 grid gap-3 sm:grid-cols-[1fr_150px]"><div><label className="block text-xs font-semibold">Template<select value={step.template_id} onChange={(event) => updateStep(index, { template_id: event.target.value })} className="mt-2 h-10 w-full rounded-md border border-slate-200 bg-white px-3 text-sm"><option value="">Select a saved template</option>{templates.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><Button type="button" variant="ghost" size="sm" onClick={() => openTemplateCreator(index)} className="mt-1 gap-1 px-0 text-xs text-blue-700 hover:text-blue-800"><Plus size={12} /> Create new template for this step</Button></div>{index > 0 && <label className="block text-xs font-semibold">Delay<Input type="number" min={1} value={step.delay_days} onChange={(event) => updateStep(index, { delay_days: Number(event.target.value) })} className="mt-2 h-10" /></label>}</div>{template && <div className="mt-3 rounded-md bg-slate-50 p-3"><p className="text-xs font-semibold">{previewValue(template.subject, selectedPreview)}</p><p className="mt-2 whitespace-pre-wrap text-xs leading-5 text-slate-600">{previewValue(template.body, selectedPreview)}</p></div>}</div>; })}</div></Surface>
    ...
    {templateCreatorOpen && <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 p-4" role="dialog" aria-modal="true" aria-label="Create email template"><div className="w-full max-w-2xl rounded-xl border border-slate-200 bg-white shadow-2xl"><div className="flex items-center justify-between border-b border-slate-200 px-5 py-4"><div><h2 className="text-lg font-semibold">Create email template</h2><p className="mt-1 text-xs text-slate-500">Create a reusable template and Rohly will select it for {steps[templateCreatorStep]?.label || "this step"}.</p></div><Button type="button" variant="ghost" size="icon" onClick={() => setTemplateCreatorOpen(false)}><X size={17} /></Button></div><div className="space-y-4 p-5"><label className="block text-xs font-semibold">Template name<Input value={newTemplateName} onChange={(event) => setNewTemplateName(event.target.value)} placeholder="e.g. Initial leather buyer outreach" className="mt-2 h-10" /></label><label className="block text-xs font-semibold">Subject<Input value={newTemplateSubject} onChange={(event) => setNewTemplateSubject(event.target.value)} placeholder="Hi {{first_name}} — quick question" className="mt-2 h-10" /></label><label className="block text-xs font-semibold">Email body<textarea value={newTemplateBody} onChange={(event) => setNewTemplateBody(event.target.value)} placeholder={'Hi {{first_name}},\n\nI wanted to reach out about {{company}}...'} rows={10} className="mt-2 w-full rounded-md border border-slate-200 bg-white px-3 py-2 text-sm leading-6 outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500" /></label><div className="rounded-md bg-blue-50 p-3 text-xs leading-5 text-blue-800"><strong>Personalization:</strong> You can use {{first_name}}, {{name}}, {{email}}, {{company}}, {{job_title}}, {{industry}}, {{city}}, and {{country}}.</div></div><div className="flex justify-end gap-2 border-t border-slate-200 px-5 py-3"><Button type="button" variant="outline" onClick={() => setTemplateCreatorOpen(false)}>Cancel</Button><Button type="button" disabled={createTemplateMutation.isPending} onClick={() => { if (!newTemplateName.trim()) return toast.error("Enter a template name"); if (!newTemplateSubject.trim()) return toast.error("Enter a subject"); if (!newTemplateBody.trim()) return toast.error("Enter an email body"); createTemplateMutation.mutate(); }}>{createTemplateMutation.isPending ? "Creating…" : "Create & select template"}</Button></div></div></div>}
    {listPickerOpen && <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 p-4" role="dialog" aria-modal="true" aria-label="Choose recipient list"><div className="w-full max-w-3xl rounded-xl border border-slate-200 bg-white shadow-2xl"><div className="flex items-center justify-between gap-4 border-b border-slate-200 px-5 py-4"><div><h2 className="text-lg font-semibold">Choose a recipient list</h2><p className="mt-1 text-xs text-slate-500">Add an existing CSV list to this campaign. You can also upload a new list and refresh this window.</p></div><div className="flex items-center gap-2"><Button type="button" variant="outline" size="sm" onClick={refreshLists} className="gap-1"><RefreshCw size={13} /> Refresh lists</Button><a href="/imports" target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 rounded-md bg-blue-700 px-3 py-1.5 text-xs font-semibold text-white hover:bg-blue-800"><Plus size={13} /> Upload new list</a><Button type="button" variant="ghost" size="icon" onClick={() => setListPickerOpen(false)}><X size={17} /></Button></div></div><div className="max-h-[60vh] overflow-auto p-5">{listsQuery.isLoading ? <p className="py-10 text-center text-sm text-slate-500">Loading imported lists…</p> : lists.length ? <div className="space-y-2">{lists.map((list) => { const active = selectedLists.includes(list.id); const busy = useListMutation.isPending && useListMutation.variables === list.id; return <div key={list.id} className={`flex items-center gap-3 rounded-lg border p-4 ${active ? "border-blue-300 bg-blue-50/50" : "border-slate-200"}`}><div className="flex size-9 items-center justify-center rounded-md bg-emerald-50 text-emerald-700"><FileSpreadsheet size={15} /></div><div className="min-w-0 flex-1"><p className="truncate text-sm font-semibold">{list.filename}</p><p className="mt-1 text-xs text-slate-500">{list.row_count} contacts · {list.columns.slice(0, 5).join(" · ")}{list.columns.length > 5 ? " …" : ""}</p></div><Button type="button" size="sm" variant={active ? "outline" : "default"} disabled={busy || active} onClick={() => useListMutation.mutate(list.id)} className="gap-1">{active ? <><Check size={13} /> Added</> : busy ? "Adding…" : "Add list"}</Button></div>; })}</div> : <div className="py-10 text-center"><p className="text-sm font-medium text-slate-700">No imported lists yet</p><p className="mt-1 text-xs text-slate-500">Upload a CSV with the button above, then click Refresh lists.</p><a href="/imports" target="_blank" rel="noreferrer" className="mt-4 inline-flex items-center gap-1 text-xs font-semibold text-blue-700"><Plus size={13} /> Upload a new list</a></div>}</div><div className="flex justify-end border-t border-slate-200 px-5 py-3"><Button type="button" variant="outline" onClick={() => setListPickerOpen(false)}>Done</Button></div></div></div>}
  </div>;
}